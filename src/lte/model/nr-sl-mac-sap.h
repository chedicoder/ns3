...
class NrSlMacSapProvider
{
  public:
    ...
    struct NrSlRlcPduParameters
    {
         ...
         */
        NrSlRlcPduParameters(Ptr<Packet> pdu,
                             uint16_t rnti,
                             uint8_t lcId,
                             uint8_t layer,
        ...
        uint8_t layer{
            std::numeric_limits<uint8_t>::max()}; //!< the layer value that was passed by the MAC in
                                                  //!< the call to NotifyNrSlTxOpportunity that
                                                  //!< generated this PDU
        uint8_t harqProcessId{
            std::numeric_limits<uint8_t>::max()}; //!< the HARQ process id that was passed by the
                                                  //!< MAC in the call to NotifyNrSlTxOpportunity
                                                  //!< that generated this PDU
        uint8_t bwpId{
            std::numeric_limits<uint8_t>::max()}; //!< the component carrier id corresponding to the
                                                  //!< sending Mac instance */
    ...
    struct NrSlReportBufferStatusParameters
    {
         ...
         * \param dstL2Id Sidelink destination L2 ID (24 bits)
         */
        NrSlReportBufferStatusParameters(uint16_t rnti,
                                         uint8_t lcId,
        ...
        uint16_t txQueueHolDelay{
            std::numeric_limits<uint16_t>::max()}; //!< the Head Of Line delay of the transmission
                                                   //!< queue
        uint32_t retxQueueSize{0}; //!< the current size of the RLC retransmission queue in bytes
        uint16_t retxQueueHolDelay{
            std::numeric_limits<uint16_t>::max()}; //!< the Head Of Line delay of the retransmission
                                                   //!< queue
        uint16_t statusPduSize{
            0};              //!< the current size of the pending STATUS RLC  PDU message in bytes
        uint32_t srcL2Id{0}; //!< Source L2 ID (24 bits)
        uint32_t dstL2Id{0}; //!< Destination L2 ID (24 bits)
    };
...
template <class C>
void
MemberNrSlMacSapProvider<C>::TransmitNrSlRlcPdu(const NrSlRlcPduParameters& params)
{
...
template <class C>
void
MemberNrSlMacSapProvider<C>::ReportNrSlBufferStatus(const NrSlReportBufferStatusParameters& params)
{
...
class NrSlMacSapUser
{
  public:
    ...
    struct NrSlTxOpportunityParameters
    {
         ...
         * \param dstL2Id Sidelink destination L2 ID (24 bits)
         */
        NrSlTxOpportunityParameters(uint32_t bytes,
                                    uint16_t rnti,
                                    uint8_t lcId,
                                    uint8_t layer,
                                    uint8_t harqId,
    ...
    struct NrSlReceiveRlcPduParameters
    {
         ...
         */
        NrSlReceiveRlcPduParameters(const Ptr<Packet>& p,
                                    uint16_t rnti,
                                    uint8_t lcid,
                                    uint32_t srcL2Id,
                                    uint32_t dstL2Id)
...
template <class C>
void
MemberNrSlMacSapUser<C>::NotifyNrSlTxOpportunity(const NrSlTxOpportunityParameters& params)
{
...
template <class C>
void
MemberNrSlMacSapUser<C>::ReceiveNrSlRlcPdu(NrSlReceiveRlcPduParameters params)
{
